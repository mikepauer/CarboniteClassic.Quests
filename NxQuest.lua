...
-- Hacks to fix Classic UI QuestInfo calling removed functions
--
...
function CarboniteQuest:OnInitialize()
	...
	Nx.Button.TypeData["QuestWatching"] = {
		Bool = true,
		Up = "Interface\\Addons\\CarboniteClassic\\Gfx\\Buttons\\DotOn",
		Dn = "Interface\\Addons\\CarboniteClassic\\Gfx\\Buttons\\DotOn",
		SizeUp = 11,
		SizeDn = 11,
		VRGBAUp = "1|1|.25|.5",
	...
	Nx.Button.TypeData["QuestWatchGivers"] = {
		Tip = L["Quest Givers"],
		States = 3,
		Tx = "Interface\\Addons\\CarboniteClassic\\Gfx\\Buttons\\DotOn",
		{
			Size = 10,
			VRGBA = "1|.811|.25|.56",
		},
	...
	Nx.Button.TypeData["QuestWatchAC"] = {		-- Auto complete
		Up = "Interface\\Addons\\CarboniteClassic\\Gfx\\Map\\IconQuestion",
		SizeUp = 15,
		VRGBAUp = ".75|1|.75|1",
	}
	...
	function ToggleQuestLog(...)
		...
		if orig then			
			...
			if self.InShowUIPanel then
				Nx.Quest:HideUIPanel(QuestLogFrame)
				self.InShowUIPanel = false
			else
				Nx.Quest:ShowUIPanel(QuestLogFrame)
...
--			end
			...
			local qId, qLevel = questID, level
...
--		Nx.prt ("skip")
	...
	if self.LHAttached then
		...
		if Nx.Map:GetMap (1).Frm.NxMap.Tick % 20 == 0 then
...
--	Nx.prt ("QuestListUpdate")
	...
	self.Win:SetTitle (format (L["Quests:"] .. " |cffffffff%d/%d|r", i, MAX_QUESTS))
		...
		if cur.Q and cur.CompleteMerge then
			...
			if mapId then
				...
				if map:ClipFrameByMapType (f, wx, wy, navscale, navscale, 0) then
					...
					f.texture:SetTexture ("Interface\\AddOns\\CarboniteClassic\\Gfx\\Map\\IconQuestion")
				end
			end
		end
	end
			...
			if not (cur and (cur.QI > 0 or cur.Party)) then
				...
				if mapId then
					...
					if map:ClipFrameByMapType (f, wx, wy, navscale, navscale, 0) then
						...
						f.texture:SetTexture ("Interface\\AddOns\\CarboniteClassic\\Gfx\\Map\\IconExclaim")
					end
				end
			else
		...
		if showEnd or bit.band (mask, 0x10000) > 0 then
			...
			if mapId and (not cur or not cur.CompleteMerge) then
				...
				if map:ClipFrameByMapType (f, wx, wy, navscale, navscale, 0) then
					...
					f.texture:SetTexture ("Interface\\AddOns\\CarboniteClassic\\Gfx\\Map\\IconQuestion")
--					f.texture:SetTexture ("Interface\\AddOns\\CarboniteClassic\\Gfx\\Map\\IconQTarget")
				end
			end
		end
...
--							Nx.prt ("%s, pt %s", objName, strsub (obj, loc + 1))
							...
							if map:ClipFrameByMapType (f, wx, wy, sz, sz, 0) then
								...
								if cnt == 1 then
									...
									f.texture:SetTexture ("Interface\\AddOns\\CarboniteClassic\\Gfx\\Map\\IconCirclePlus")
...
--							Nx.prt ("%s, spans %s", objName, strsub (obj, loc))
							...
							if x then
								...
								if map:ClipFrameByMapType (f, x, y, sz, sz, 0) then
									...
									f.texture:SetTexture ("Interface\\AddOns\\CarboniteClassic\\Gfx\\Map\\IconAreaArrows")
...
function Nx.Quest:UpdateQuestDetailsTimer()
	...
	QuestInfo_Display (CBQUEST_TEMPLATE, NXQuestLogDetailScrollChildFrame,nil,nil,nil)
